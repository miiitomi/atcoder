...
template<typename X, typename M> struct LazySegmentTree {
    ...
    X X_M_operation(X x, M m) {
        ...
        return {m.first*x.first + m.second*x.second, x.second};
    }
...
void solve() {
    ll N, Q, X, I = 0;
    cin >> N >> Q >> X;
    LazySegmentTree<pair<ll,ll>, pair<ll,ll>> lst(N);
    for (int i = 0; i < N; i++) {
        int p;
        cin >> p;
        if (p < X) lst.initialize(i, {1, 1});
        else lst.initialize(i, {0, 1});
        if (p == X) I = i;
    }
    while (Q--) {
        ll c, l, r;
        cin >> c >> l >> r;
        l--;
        ll m = lst.query(l, r).first;
        if (c == 1) {
            if (l <= I && I < r) {
                I = l + m;
            }
            lst.update(l, r, make_pair(0LL, 0LL));
            lst.update(l, l+m, make_pair(0LL, 1LL));
        } else {
            if (l <= I && I < r) {
                I = r - 1 - m;
            }
            lst.update(l, r, make_pair(0LL, 0LL));
            lst.update(r-m, r, make_pair(0LL, 1LL));
        }
    }
    cout << I+1 << endl;
}
