...
int main() {
    ...
    while (t <= 5900) {
        ...
        double r = Randouble();
        double diff = 1.0 - ((double)fuman_1 - (double)fuman_0) / (double)fuman_1;
        double threshold = (1.0 - exp(-diff/(t/10.0 + 1.0))) / 2.0; 

        if (fuman_0 < fuman_1 && r > threshold) {
            swap(edge_lists[i].day, edge_lists[j].day);
        } else {
