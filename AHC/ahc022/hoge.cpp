...
struct Simulator {
    ...
    void set_P() {
        ...
        int t = min((int)(((double)p*high_value)/((double)p + (int)Q_zero.size())), 1000);
        for (int i = 0; i < L; i++) {
            for (int j = 0; j < L; j++) {
                pair<int,int> pt = make_pair(i,j);
                if (Q.count(pt)) P[i][j] = 1000;
                else if (Q_zero.count(pt)) P[i][j] = 500;
                // else P[i][j] = t;
            }
        }
        ...
        t = (int)chrono::duration_cast<chrono::milliseconds>(end_t-start).count();
        // cout << "# " << t << " [ms]" << endl;
    }
