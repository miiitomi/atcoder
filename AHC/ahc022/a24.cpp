...
struct Simulator {
    ...
    void set_P(mt19937 &engine) {
        ...
        int max_p = min(12, (int)sqrt((50.0*N*N*L)/(4.0*high_value*high_value)) - 1);
        int l=1, r=max_p+1;
        ...
        while (r-l>1) {
            ...
            while (int_t < 3000/k_) {
                ...
                for (int k = 0; k < N; k++) {
                    ...
                    tmp_cost += 100*(10+abs(min_x)+abs(min_y));
    ...
    Simulator(int l_, int n_, int s_, mt19937 &engine) {
        ...
        if ((int)sqrt(S) <= 15) high_value = high_values[(int)sqrt(S)-1];
