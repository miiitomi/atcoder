...
struct Simulator {
    ...
    void solve(bool _final_phase, mt19937 engine) {
        ...
        for (int i = 0; i < N; i++) for (int k = 0; k < N; k++) V[i].push(make_pair(0, k));
        ...
        do  {
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (int t_ = 0; (t_ < remain_warp_par_entrance) && (!V[i].empty()); t_++) {
                    ...
                    if (q > max(0.0, high_value - K*S)) new_V[i].push(make_pair(R[i][k], k));
                }
            }
            ...
            V = new_V;
            try_number++;
        } while (remain_warp_par_entrance > 0 && did_warp);
