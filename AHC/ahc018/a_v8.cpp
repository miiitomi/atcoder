...
vector<vector<double>> estimated_depth;
...
int for_each_house(int i) {
    ...
    bool move_x = true;
    int old_destroy_count = 1;

    while (S[x][y] > 0 || x != to_x || y != to_y) {
        int new_destroy_count = 0;
        ...
        if (new_destroy_count > old_destroy_count) {
            move_x = !move_x;
        }
        old_destroy_count = new_destroy_count;
    }
...
int main() {
    ...
    estimated_depth.resize(N);
    for (int i = 0; i < N; i++) estimated_depth[i].assign(N, 0);
