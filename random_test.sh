...
g++ ${file_no_extension}_generate.cpp -std=gnu++23 -I ${ac_library_path} -o ${file_no_extension}_generate.out
oj g/i -d ${test_dir} ./${file_no_extension}_generate.out 10
rm ${file_no_extension}_generate.out

g++ ./${file_no_extension}_naive.cpp -std=gnu++23 -I ${ac_library_path} -o ./${file_no_extension}_naive.out
oj g/o -c ./${file_no_extension}_naive.out -d ${test_dir}
rm ./${file_no_extension}_naive.out

g++ ${file} -std=gnu++23 -Wall -Wextra -O2 -I ${ac_library_path} -o ./${file_no_extension}.out
