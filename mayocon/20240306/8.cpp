...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if ((i+j) % 2 == 0) {
                if (c[i][j] == 'W') mfg.add_edge(N*N, to_int(i, j), 1e+9);
                if (c[i][j] == 'B') mfg.add_edge(to_int(i, j), N*N+1, 1e+9);
            } else {
                if (c[i][j] == 'B') mfg.add_edge(N*N, to_int(i, j), 1e+9);
                if (c[i][j] == 'W') mfg.add_edge(to_int(i, j), N*N+1, 1e+9);
            }
            for (int k = 0; k < 4; k++) {
                int x = i + dx[k], y = j + dy[k];
                if (is_in(x, y)) {
                    mfg.add_edge(to_int(i, j), to_int(x, y), 1);
                    cnt++;
                }
            }
        }
    }
    cout << cnt/2 - mfg.max_flow(N*N, N*N+1) << endl;
}
