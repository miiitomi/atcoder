...
int main() {
    ...
    int s = H+W+2*N, g = H+W+2*N+1;

    for (int i = 0; i < H; i++) mfg.add_edge(s, i, 1);
    for (int i = 0; i < W; i++) mfg.add_edge(H+i, g, 1);
    for (int i = 0; i < N; i++) {
        int a, b, c, d;
        cin >> a >> b >> c >> d;
        a--;
        ...
        c--;
        d--;
        mfg.add_edge(H+W+2*i, H+W+2*i+1, 1);
        for (int r = a; r <= c; r++) mfg.add_edge(r, H+W+2*i, 1);
        for (int x = b; x <= d; x++) mfg.add_edge(H+W+2*i+1, H+x, 1);
    }

    int ans = mfg.max_flow(s, g);
    cout << ans << endl;
}
